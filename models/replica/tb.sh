...
echo "setting LOG_DIR=${LOG_DIR}"
echo "starting tensorboard..."
# tensorboard --port=${PORT_NUM} --logdir=${LOG_DIR} --host localhost
# tensorboard --port=${PORT_NUM} --samples_per_plugin=images=100 --logdir=${LOG_DIR} --bind_all
tensorboard --port=${PORT_NUM} --samples_per_plugin=images=20 --logdir=${LOG_DIR}
