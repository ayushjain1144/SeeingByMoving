...
DATASET_PATH_TRAIN = os.path.join(DATASET_PATH_OBJECTS, 'testing') # change to testing too
...
DATASET_PATH_SMALL_KITTI = "/home/ayushj2/frustum_replica_pseudo/frustum_pointnets_pytorch/kitti"
...
output_gt_dir = "./gt_pred_vis_val"
output_maskrcnn_dir = "./maskrcnn_pred_vis_val"
output_pseudo_dir = "./pseudo_dir_vis_val"
...
class CarlaViewmineModel(nn.Module):
    def __init__(self):
        ...
        self.obj_counts = 0
        self.avg_iou = 0
        self.img_count = 0         # any safe valuw that is big enough
        self.idx_count = 0
    ...
    def run_train(self, feed):
        ...
        for s in list(range(self.S)):
            ...
            for n in list(range(N)):
                ...
                if (occ_camX0_obj_masked.sum().item() == 0):
                    print("occupancy is empty for ", self.maskrcnn_to_catname[self.obj_id_list_camXs[s][n]], " discarding...")
                    self.any_mask_list_camXs[s][self.any_mask_list_camXs[s] == obj_id] = 0
                    continue
                ...
                if hyp.do_crf:
                    ...
                    for ss in list(range(self.S)):
                        ...
                        xyz_amodal_seg = xyz_amodal.unsqueeze(0)
                        
                        # estimating 3D box in kitti format in camX frame
                        mask_grid_s = obj_vox_util.voxelize_xyz(xyz_amodal_seg, Z_dim, Y_dim, X_dim, assert_cube=False)
                        # st()
                        ...
                        if do_visualize:
                            ...
                            self.summ_writer.summ_rgbs(f'boxes3d/boxes3d_all_box{ss}', vis1)
                        ...
                        if key in all_info_list:
                            all_info_list[key].append([obj_score, mask_modal, bbox_amodal, mask_amodal, bbox_modal, box3d])
                        else:
                            all_info_list[key] = [[obj_score, mask_modal, bbox_amodal, mask_amodal, bbox_modal, box3d]]
        ...
        if do_consistency:
            ...
            for s in list(range(self.S)):
                ...
                # get all masks for inter-class NMS
                for catid in list(self.maskrcnn_to_catname.keys()):
                    key = (s, catid)
                    if key in all_info_list:
                        ...
                        # add all masks
                        for m in range(len(view_cat_list)):
                            keep_list.append([catid, view_cat_list[m][1], view_cat_list[m][2], view_cat_list[m][0], view_cat_list[m][4], view_cat_list[m][5]])
                ...
                while len(keep_list) > 0:
                    ...
                    keep_list_rem = []
                    overlapped_ids = []
                    overlapped_ids.append(keep_list[0][0])
                    for i in range(1, len(keep_list)):
                        ymin2, xmin2, ymax2, xmax2 = keep_list[i][2]
                        ...
                        if x_dist > 0 and y_dist > 0:
                            ...
                            if float(area_overlap) /float(area_conf + area_cur - area_overlap) > 0.5:
                                overlapped_ids.append(keep_list[i][0])
                                continue

                        keep_list_rem.append(keep_list[i])
                    keep_list = keep_list_rem
                    overlapped_groups.append(np.array(overlapped_ids))

                consistent_view = True
                for ids in range(len(overlapped_groups)):
                    cur_group = overlapped_groups[ids]

                    how_many_overlaps = cur_group.shape[0]
                    unique_ids = np.unique(cur_group)

                    num_overlaps_thresh = 3
                    # if number of confident masks less than 3 or masks predict different things, do not include view
                    if (how_many_overlaps < num_overlaps_thresh) or unique_ids.shape[0] > 1:
                        print("FOUND INCONSISTENT VIEW #", s)
                        consistent_view = False
                        break
                    else: # consistent group
                        pass 
                
                consistent_views.append(consistent_view)


        
        if do_consistency: # keep only consistent views
            views = []
            for idx in range(len(consistent_views)):
                if consistent_views[idx]:
                    views.append(idx)
        else:
            views = list(range(self.S))
        ...
        for s in views:
            ...
            for catid in list(self.maskrcnn_to_catname.keys()):
                ...
                if key in all_info_list:
                    ...
                    while len(view_cat_list) > 0:
                        ...
                        keep_list.append([catid, view_cat_list[0][1], view_cat_list[0][2], view_cat_list[0][0], view_cat_list[0][4], view_cat_list[0][5]])
            ...
            while len(keep_list) > 0:
                ...
                box3d_list.append(keep_list[0][5])
            ...
            if do_map_eval:
                ...
                for i in range(len(boxlist_e_pseudo)):
                    ...
                    pseudo_file.write(f"{self.maskrcnn_to_catname[int(class_list_e_pseudo[i])]} 1 {round(boxlist_e_pseudo[i][1])} {round(boxlist_e_pseudo[i][0])} {round(boxlist_e_pseudo[i][3])} {round(boxlist_e_pseudo[i][2])}\n")
                pseudo_file.close()
            ...
            if make_pointnet_dataset:
                ...
                with open(os.path.join(DATASET_PATH_KITTI_LABELS, f'{self.idx_count}.txt'), 'w') as f:
                    for i in range(len(catid_list)):
                        # st()
                        type_name = self.maskrcnn_to_catname[int(class_list_e_pseudo[i])]
                ...
                # write image sets
                with open(os.path.join(DATASTET_PATH_IMAGE_SETS, f'test.txt'), 'a+') as f:
                    f.write(f"{self.idx_count}\n")

                with open(os.path.join(DATASET_PATH_IMAGESETS, f'test.txt'), 'a+') as f:
                    f.write(f"{self.idx_count}\n")
