...
g_type2class={'chair':0, 'bed':1, 'toilet':2, 'couch':3, 'refrigerator':4, 'tv':5, 'indoor-plant':6, 'dining-table':7}
g_class2type = {g_type2class[t]:t for t in g_type2class}
g_type2onehotclass = {'chair':0, 'bed':1, 'toilet':2, 'couch':3, 'refrigerator':4, 'tv':5, 'indoor-plant':6, 'dining-table':7}
...
g_type_mean_size = {'chair' : np.array([1.0476828 , 1.39355489, 1.40725195]),
                    'bed' :   np.array([3.06105234, 2.86818274, 3.03759278]),
                    'toilet' : np.array([0.80274984, 1.05743585, 1.0455594 ]),
                    'couch' : np.array([2.72364379, 2.32685216, 2.6531894 ]),
                    'refrigerator' : np.array([0.95466657, 2.03380304, 1.68343477]),
                    'tv' : np.array([1.48194694, 1.33844123, 1.49796456]),
                    'indoor-plant' : np.array([0.87121925, 1.20293167, 1.17741687]),
                    'dining-table' : np.array([1.12939206, 1.10197106, 1.20573152])}
...
class PointNetInstanceSeg(nn.Module):
    def __init__(self,n_classes=8,n_channel=3):
...
class FrustumPointNetv1(nn.Module):
    def __init__(self,n_classes=8,n_channel=3):
        ...
        self.n_classes = n_classes
        self.InsSeg = PointNetInstanceSeg(n_classes=8,n_channel=3)
        self.STN = STNxyz(n_classes=8)
        self.est = PointNetEstimation(n_classes=8)
