...
def get_mAP_with_classes(boxlist_e, boxlist_g, class_list_e, class_list_g, confidence_list, num_classes=2, mode="coco"):
    # maskrcnn prediction: [xmin, ymin, xmax, ymax, class_id, confidence]
    # gt: [xmin, ymin, xmax, ymax, class_id, difficult, crowd]
    # https://pypi.org/project/mean-average-precision/

    """
    boxlist_e = boxlist predicted by maskrcnn in the format our utils expect (ymin,xmin,ymax,xmax)
    boxlist_g = boxlist in gt. https://bitbucket.org/adamharley/pytorch_disco/src/entity_bottleneck_active/model_carla_viewmine.py this has all the code for it.
    class_list_e = class_id list of objects in the scene. keep it 0 or 1. 0 = motorcycle; 1 = other vehicles
    confidence_list = create this list from detectron2 confidence
    num_classes: should be 2 for our case
    mode: we have two modes: "pascal" for pascal voc mAP,  and "coco" for coco mAP.
    """

    preds = []
    for idx, pred_box in enumerate(boxlist_e):
        ymin,xmin,ymax,xmax = pred_box
        class_id = class_list_e[idx]
    ...
    gts = []
    for idx, gt_box in enumerate(boxlist_g):
        ymin,xmin,ymax,xmax = gt_box
        class_id = class_list_g[idx]
        difficult = 0
        crowd = 0
