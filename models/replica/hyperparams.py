...
saved_ckpt_dir = "/projects/katefgroup/checkpoints_viewpredseg/"
...
N = 3 # number of boxes produced by the rcnn (not all are good)
K = 3 # number of boxes to actually use
S = 2 # seq length
S_test = 3 # seq length
...
do_freeze_up2d = False
do_freeze_slot = False
do_freeze_cut = False
...
do_carla_slot2d = False
do_carla_slot3d = False
do_carla_proposal = False
do_nuscenes_slot3d = False
...
elif mode=="CARLA_SLOT2D":
    exec(compile(open('exp_carla_slot2d.py').read(), 'exp_carla_slot2d.py', 'exec'))
elif mode=="CARLA_SLOT3D":
    exec(compile(open('exp_carla_slot3d.py').read(), 'exp_carla_slot3d.py', 'exec'))
elif mode=="CARLA_PROPOSAL":
    exec(compile(open('exp_carla_proposal.py').read(), 'exp_carla_proposal.py', 'exec'))
elif mode=="NUSCENES_SLOT3D":
    exec(compile(open('exp_nuscenes_slot3d.py').read(), 'exp_nuscenes_slot3d.py', 'exec'))
...
elif mode=="NUSCENES_COMPOSE":
    exec(compile(open('exp_nuscenes_compose.py').read(), 'exp_nuscenes_compose.py', 'exec'))
elif mode=="CARLA_OCC":
...
elif mode=="CARLA_VIEWMINE":
    exec(compile(open('exp_carla_viewmine.py').read(), 'exp_carla_viewmine.py', 'exec'))
elif mode=="CARLA_EVAL":
    exec(compile(open('exp_carla_eval.py').read(), 'exp_carla_eval.py', 'exec'))
elif mode=="CARLA_GT":
    # st()
    exec(compile(open('exp_carla_gt.py').read(), 'exp_carla_gt.py', 'exec'))
elif mode=="CARLA_SEMI":
    exec(compile(open('exp_carla_semi.py').read(), 'exp_carla_semi.py', 'exec'))
else:
...
if do_test:
    ...
    if do_preocc or do_feat3d or do_render or do_sigen3d or do_oct or do_bkg or do_occ:
        name += "_m%dx%dx%d" % (Z, Y, X)
    # if do_forecast or do_match:
    #     name += "_z%dx%dx%d" % (ZZ, ZY, ZX)
    if do_forecast or do_rigid or do_robust:
        name += "_z%dx%dx%d" % (ZZ, ZY, ZX)
